...
wire ReadWrite_top, ALUSrc_top,zero_top,branch_top,sel2_top,MemtoReg_top,MemWrite_top,MemRead_top;
wire [1:0]ALUOp_top;
...
reg_file reg_file (.clk(clk),.reset(reset),.RegWrite(ReadWrite_top),.rs1(instruction_top[19:15]),.rs2(instruction_top[24:20]),.RD(instruction_top[11:7]),.Write_data(WriteBack_top),.read_data1(Rd1_top),.read_data2(Rd2_top));

ImmGen ImmGen(.Opcode(instruction_top[6:0]) , .instruction(instruction_top), .ImmExt(ImmExt_top));
Control_Unit Control_Unit(.instruction(instruction_top[6:0]), .Branch(branch_top) , .MemRead(MemRead_top), .MemtoReg(MemtoReg_top), .ALUop(ALUOp_top), .MemWrite(MemWrite_top), .ALUSrc(ALUSrc_top), .RegWrite(ReadWrite_top));
